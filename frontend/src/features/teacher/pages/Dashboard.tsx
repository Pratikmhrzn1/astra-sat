...
export default function TeacherDashboard() {
  ...
  const statCards = [
    ...
    { label: 'Feedback Sent', value: sentFeedback.length, icon: MessageSquare, bg: 'rgba(226,86,43,0.08)', color: '#C4471F' },
  ...
  return (
    <div className="screen-fade" style={{ padding: '36px 48px 64px' }}>
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontSize: 13, color: 'rgba(11,11,14,0.58)', marginBottom: 4 }}>Teacher Portal</div>
        <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 44, margin: 0, letterSpacing: '-0.02em', color: '#0B0B0E' }}>Welcome, {user?.name}</h1>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 28 }}>
        {statCards.map(({ label, value, icon: Icon, bg, color }) => (
          <div key={label} style={{ background: '#fff', border: '1px solid #E7E4DE', borderRadius: 16, boxShadow: '0 1px 3px rgba(11,11,14,0.05)', padding: '20px 22px', display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div>
              <div style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 38, lineHeight: 1, color: '#0B0B0E' }}>{value}</div>
      ...
      <div style={{ background: '#fff', border: '1px solid #E7E4DE', borderRadius: 16, boxShadow: '0 1px 3px rgba(11,11,14,0.05)', overflow: 'hidden' }}>
        <div style={{ padding: '16px 22px', borderBottom: '1px solid #EEEBE5', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {students.length > 0 && (
            <button onClick={() => navigate('/teacher/students')} style={{ fontSize: 13, fontWeight: 600, color: '#C4471F', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>View all →</button>
          )}
        </div>
        {students.length === 0 ? (
          <div style={{ padding: '48px 22px', textAlign: 'center', color: 'rgba(11,11,14,0.58)', fontSize: 14 }}>No students assigned yet.</div>
        ) : (
          <div>
