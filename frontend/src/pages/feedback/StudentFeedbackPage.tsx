...
import { getFeedback, markFeedbackRead } from '@/api/student';
...
export default function StudentFeedback() {
  ...
  return (
    <div className="screen-fade" style={{ padding: '36px 48px 64px', maxWidth: 820, margin: '0 auto' }}>
      <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 44, margin: '0 0 4px', letterSpacing: '-0.02em', color: '#0B0B0E' }}>Feedback</h1>
      <p style={{ fontSize: 14, color: 'rgba(11,11,14,0.64)', margin: '0 0 24px' }}>Messages from your teacher</p>

      {feedbacks.length === 0 ? (
        <div style={{ textAlign: 'center', paddingTop: 64 }}>
          <MessageSquare size={48} color="rgba(11,11,14,0.2)" style={{ margin: '0 auto 16px', display: 'block' }} />
          <p style={{ color: 'rgba(11,11,14,0.58)', fontSize: 14 }}>No feedback yet.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {feedbacks.map((fb) => (
            <div
              ...
              onPointerEnter={(e) => { if (e.pointerType !== 'mouse') return; e.currentTarget.style.background = '#FBFAF8'; }}
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 10 }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: 'rgba(11,11,14,0.58)' }}>{fb.teacherEmail}</div>
                </div>
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{ fontSize: 12, color: 'rgba(11,11,14,0.58)' }}>{formatDateTime(fb.createdAt)}</div>
                  {!fb.isRead && (
                    <span style={{ display: 'inline-block', marginTop: 4, padding: '2px 8px', background: 'rgba(226,86,43,0.1)', color: '#C4471F', fontSize: 11, fontWeight: 700, borderRadius: 9999, letterSpacing: '0.05em' }}>NEW</span>
                  )}
                </div>
              </div>
              <p style={{ fontSize: 14, color: 'rgba(11,11,14,0.7)', lineHeight: 1.6, margin: 0, whiteSpace: 'pre-wrap' }}>{fb.content}</p>
              {fb.examId && (
                <p style={{ fontSize: 12, color: 'rgba(11,11,14,0.58)', marginTop: 8, marginBottom: 0 }}>Related to an exam</p>
              )}
            </div>
          ))}
        </div>
